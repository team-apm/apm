...
#### 一覧に無いパッケージを自分で追加する場合（ローカルリポジトリ）

apmは、既定のパッケージデータに加えて**AviUtlフォルダ内に置いたデータ**も読み込みます。apm-dataへ登録しなくても、自分の環境にだけパッケージを追加できます。

方法は2つあります。

**設定タブの「追加テキストデータ」を使う**

1. 「設定」タブを開き、「追加テキストデータ」のエディタに[apm-dataの形式](https://github.com/team-apm/apm-data)でパッケージデータを書きます。
2. 「保存」ボタンを押すと、AviUtlフォルダ内の`editorPackages.json`に保存されます。
3. 「プラグイン&スクリプト」タブに戻ると、書いたパッケージが一覧に現れ、通常のパッケージと同じようにインストールできます。

**`packages.json`を直接置く**

AviUtlフォルダの直下に`packages.json`を置くと、同じように読み込まれます。ファイルで管理したい場合や、他の人と共有したい場合に向いています。

いずれの場合も、一覧への表示・インストール・アンインストールは通常のパッケージと同じように動作します。

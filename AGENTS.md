...
- **自身の `__dirname` からファイルを読む依存はバンドルできない**(`7zip-bin` / `win-7zip` の実行ファイル、`electron-prompt` の HTML)。`vite.base.config.ts` の `assetBearingDependencies` に挙げて external にし、`forge.config.ts` の `packagedPaths` で `node_modules` ごと同梱する。この一覧から漏れるとパッケージ版だけが `MODULE_NOT_FOUND` で落ちるが、**Electron が main のロード例外をダイアログに出すため stderr には何も出ず「窓が開かない」としか見えない**。`assertExternals` プラグインがビルド時に落とすので、そのエラーが出たら「バンドルできるよう静的 import に直す」か「一覧に足す」かを選ぶ(生の `require()` は Vite が解決できず外部参照として残るので、原則は静的 import へ直す)
